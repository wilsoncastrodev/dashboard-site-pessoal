...
import { sortCategoriesSkill } from "../../stores/features/categorySkillSlice";
import { MDCSnackbar } from "@material/snackbar";

const DialogSortCategorySkill = ({ categoriesSkill }: any) => {
    ...
    const setSkillsSort = (categories: any) => {
        let categoriesSkillSort;

        categoriesSkillSort = categories.map((item: any) => {
            let _item: any = { ...item };
        ...
        return {
            categoriesSkill: categoriesSkillSort,
        };
    }
    ...
    const footerContent = (
        <div>
            <Button label="Salvar"
                    ...
                    onClick={async () => {
                            const payload:any = setSkillsSort(items);
                            await dispatch(sortCategoriesSkill(payload));
                            setVisible(false);
                ...
                <Row>
                    <Col>
                        <DataTable value={items} className="datatable-order"
                            reorderableRows
                            onRowReorder={(e) => {
                                let _categoriesSkill = e.value.map((category:any, key:any) => {
                                    let _category: any = { ...category };
                                    _category.order = key;
                                    return _category;
                                });

                                setItems(_categoriesSkill);
                            }}>
...
export default DialogSortCategorySkill;
